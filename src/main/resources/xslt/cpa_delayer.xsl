...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!--<xsl:template match="file[@heading and @level = 'Level 3']">
        
        <xsl:variable name="heading" select="tokenize(@heading, '\s+')"/>
        <xsl:variable name="first_10_words" select="string-join($heading[position() &lt;= 10], ' ')"/>
        <xsl:variable name="Title"
            select="translate($first_10_words, translate($first_10_words, 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789 ', ''), '')"/>
        <xsl:variable name="title_heading" select="replace($Title, ' ','_')"/>
        
        <xsl:result-document href="cpa/{$title_heading}{generate-id()}.dita">
